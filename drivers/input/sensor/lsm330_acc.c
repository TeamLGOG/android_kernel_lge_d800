...
static void lsm330_acc_input_work_func(struct work_struct *work)
{
	...
	acc = container_of(work, struct lsm330_acc_data, input_work.work);
...
int lsm330_acc_power_on(struct lsm330_acc_platform_data *pdata)
{
	int err ;

	err = regulator_enable(pdata->vdd_ana);
	err = regulator_enable(pdata->vdd_i2c);
	printk(KERN_INFO "lsm330_acc_power_on call");

	return err ;

}

int lsm330_acc_power_off(struct lsm330_acc_platform_data *pdata)
{
	int err ;

	err = regulator_disable(pdata->vdd_ana);
	err = regulator_disable(pdata->vdd_i2c);
	printk(KERN_INFO "lsm330_acc_power_off call");
...
static int lsm330_acc_parse_dt(struct device *dev, struct lsm330_acc_platform_data *pdata)
{
	...
	pdata->init = NULL;
	pdata->exit = NULL;
	pdata->power_on = lsm330_acc_power_on;
	pdata->power_off = lsm330_acc_power_off;
	/*                     */
	/*
    ...
    */
	...
	if (client->dev.of_node) {
		...
		if (client->dev.platform_data == NULL) {
			pr_info("using default platform_data for accel\n");
