...
#define AKM8963_DEBUG_IF	1
#define AKM8963_DEBUG_DATA	0

/*                           */
#define AKM8963_INTPUT_NOTIFY
...
struct akm8963_data {
	...
	struct akm8963_platform_data *platform_data;
	...
	char const *buf, size_t count)
{
	...
	count = akm8963_sysfs_enable_store(
				dev_get_drvdata(dev), buf, count, MAG_DATA_FLAG);

#ifdef AKM8963_INTPUT_NOTIFY
	if (!atomic_read(&akm->active)) {
		/*                                                                        */
		atomic_set(&akm->is_busy, 0);
		AKECS_Set_CNTL1(akm, 0x00);
	} else {
		/*                                                                        */
		atomic_set(&akm->is_busy, 0);
		AKECS_Set_CNTL1(akm, 0x02);
	}
#endif
	return count;
}
...
/*                                           */
	...
	err = AKECS_SetMode(akm, AK8963_MODE_SNG_MEASURE);
	if (err < 0)
		return err;
...
static DEVICE_ATTR(enable, S_IWUGO | S_IRUGO,
		akm8963_enable_mag_show, akm8963_enable_mag_store);
static DEVICE_ATTR(poll_delay, S_IWUGO | S_IRUGO,
		akm8963_delay_mag_show, akm8963_delay_mag_store);
...
#ifdef AKM8963_INTPUT_NOTIFY
	raw[0] = ((s16) ((s16) akm->sense_data[1] << 8)) | akm->sense_data[2];
	raw[1] = ((s16) ((s16) akm->sense_data[3] << 8)) | akm->sense_data[4];
	raw[2] = ((s16) ((s16) akm->sense_data[5] << 8)) | akm->sense_data[6];

	input_report_abs(akm->input, ABS_RX, raw[0]);
	input_report_abs(akm->input, ABS_RY, raw[1]);
	input_report_abs(akm->input, ABS_RZ, raw[2]);
	input_report_abs(akm->input, ABS_RUDDER, 0);
	input_sync(akm->input);
#endif
...
{
	struct akm8963_data *akm = dev_get_drvdata(dev);
	struct akm8963_platform_data *pdata = akm->platform_data;
...
static int sensor_regulator_configure(struct akm8963_data *data, bool on)
{
	struct i2c_client *client = data->i2c;
	struct akm8963_platform_data *pdata = data->platform_data;
	int rc;

	if (on == false)
		goto hw_shutdown;

	pdata->vcc_ana = regulator_get(&client->dev, "Ashai,vdd_ana");
	...
	if (pdata->digital_pwr_regulator) {
		pdata->vcc_dig = regulator_get(&client->dev, "Ashai,vddio_dig");
	...
	if (pdata->i2c_pull_up) {
		pdata->vcc_i2c = regulator_get(&client->dev, "Ashai,vddio_i2c");
...
static int sensor_regulator_power_on(struct akm8963_data *data, bool on)
{
	struct i2c_client *client = data->i2c;
	struct akm8963_platform_data *pdata = data->platform_data;
	int rc;
...
{
	sensor_regulator_power_on(s_akm, on);
	return 0;
}
...
static int sensor_platform_hw_init(void)
{
	...
	struct akm8963_data *data = s_akm;
	int error;
	...
	if (gpio_is_valid(data->platform_data->irq_gpio)) {
		/*                                */
		error = gpio_request(data->platform_data->irq_gpio, "akm_irq_gpio");
		if (error) {
...
static int sensor_parse_dt(struct device *dev, struct akm8963_platform_data *pdata)
{
	struct device_node *np = dev->of_node;

	/*                */
	pdata->i2c_pull_up = of_property_read_bool(np, "Ashai,i2c-pull-up");
	pdata->digital_pwr_regulator = of_property_read_bool(np,
						"Ashai,dig-reg-support");

	/*                      */
	pdata->irq_gpio = of_get_named_gpio_flags(np, "Ashai,irq-gpio",
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		pdata = devm_kzalloc(&client->dev,
			sizeof(struct akm8963_platform_data), GFP_KERNEL);
		if (!pdata) {
			dev_err(&client->dev, "Failed to allocate memory\n");
			return -ENOMEM;
		}

		s_akm->platform_data = pdata;
	...
	err = device_create_file(&client->dev, &dev_attr_poll_delay);
	if (0 > err) {
		dev_err(&client->dev,
			"%s: create device sysfs failed.", __func__);
		goto exit6;
	}
...
{
	struct akm8963_data *data = i2c_get_clientdata(client);
	struct akm8963_platform_data *pdata = data->platform_data;
