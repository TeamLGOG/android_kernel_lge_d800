...
int mms100_download(int download_type, int file_download)
{
	...
	if (file_download == external_img)
		...
		if (download_type == isc_type) {
			...
			ret = mcsdl_download((const UINT8 *) MELFAS_binary, (const UINT16)MELFAS_binary_nLength , 0);  /*                                    */
		} else if (download_type == check_type) {
			printk("<MELFAS> ISP download check\n");
			ret = mcsdl_download((const UINT8 *) test_binary, (const UINT16)test_nLength , 0);  /*                                    */
		} else {
			printk("<MELFAS> ISP download for sys \n");
			ret = mcsdl_download_sys((const UINT8 *) MELFAS_binary, (const UINT16)MELFAS_binary_nLength , 0);  /*                                    */
		}
#endif
	}
...
int mms100_ISC_download_binary_file(void)
{
	...
	nRead = sys_read(fd, (char __user *)pBinary, nBinary_length);           /*                  */
