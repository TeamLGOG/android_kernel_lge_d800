...
void mdss_dsi_cmd_test_pattern(struct mdss_panel_data *pdata)
{
	...
	MIPI_OUTP((ctrl_pdata->ctrl_base) + 0x016c, 0xff0000); /*     */
	i = 0;
	while (i++ < 50) {
			...
			struct dsi_cmd_desc *cmds, int rlen, u32 rx_flags)
{
  ...
  */
   ...
   */
	...
	if (video_mode)
		MIPI_OUTP((ctrl->ctrl_base) + 0x0004, dsi_ctrl); /*         */
...
void mdss_dsi_error(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	/*                   */
	mdss_dsi_ack_err_status(base);	/*              */
	mdss_dsi_timeout_status(base);	/*              */
	mdss_dsi_fifo_status(base);		/*                 */
	mdss_dsi_status(base);		/*                */
	mdss_dsi_dln0_phy_err(base);	/*                  */
}
