...
#define REDUCE_REPEAT_PRINT_INFO 1
...
struct LVTTL_HW_Interface
{
	...
	subEmbedded_Sync sEmbedded_Sync;//            
	subYC_MUX sYC_MUX;
};
...
#define sp_write_reg_mask(address, offset, mask, reg_value) \
	do { \
		...
		SP_TX_Read_Reg(address, offset, &temp_value); \
		SP_TX_Write_Reg(address, offset, (temp_value&mask)|reg_value); \
		}while(0);
...
//        
_SP_TX_DRV_EX_C_ BYTE edid_pclk_out_of_range;//                                 
_SP_TX_DRV_EX_C_ BYTE sp_tx_edid_err_code;//                          
_SP_TX_DRV_EX_C_ BYTE bEDIDBreak; //                 


//        

_SP_TX_DRV_EX_C_ BYTE sp_tx_pd_mode; //                        
//                    
_SP_TX_DRV_EX_C_ WORD bBIST_FORMAT_INDEX;
...
_SP_TX_DRV_EX_C_ BYTE SP_TX_EDID_PREFERRED[18];//              
_SP_TX_DRV_EX_C_ BYTE sp_tx_ds_edid_hdmi; //                       
_SP_TX_DRV_EX_C_ BYTE sp_tx_ds_edid_3d_present;//                             
_SP_TX_DRV_EX_C_ BYTE DTDbeginAddr;
_SP_TX_DRV_EX_C_ BYTE EDID_Print_Enable;
_SP_TX_DRV_EX_C_ BYTE EDIDExtBlock[128];

_SP_TX_DRV_EX_C_ unsigned long pclk; //                        
_SP_TX_DRV_EX_C_ long int M_val,N_val;
_SP_TX_DRV_EX_C_ SP_LINK_BW sp_tx_bw;  //                     
_SP_TX_DRV_EX_C_ BYTE sp_tx_lane_count; //                        
#if(AUTO_TEST_CTS)
