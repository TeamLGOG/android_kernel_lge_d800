...
int fc8150_tuner_init(HANDLE hDevice, u32 band)
{
	...
	if(0x80<=RFPD_REF)		fc8150_write(hDevice, 0x7B, 0x8F);
	else if (RFPD_REF<0x80)		fc8150_write(hDevice, 0x7B, 0x88);
...
int fc8150_set_freq(HANDLE hDevice, band_type band, u32 rf_kHz)
{
	...
	f_vco = (rf_kHz) << 2;
	if(f_vco < bbm_xtal_freq*40)
		r_val = 2;
	else
		r_val = 1;

	f_comp = bbm_xtal_freq / r_val;
	n_val =	f_vco / f_comp;
...
int fc8150_get_rssi(HANDLE hDevice, int *rssi)
{
	...
	//                                                                                                                                      
	*rssi = (LNA & 0x07) * 6 + (RFVGA & 0x1F) + ((CSF & 0x03)+((CSF & 0x70) >> 4) )*6 - PGA / 4 + A2 * CRNTMODE1 + A3 * CRNTMODE0 + K;
	return BBM_OK;
}
