...
LGD_UINT8 LGD_PLL_SET(LGD_UINT8 ucI2CID) 
{
	...
	LGD_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x00, 0x0000);
	...
	LGD_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x00, 0x0001);
	break;
...
LGD_UINT8 LGD_INIT(LGD_UINT8 ucI2CID)
{
	...
	case EXTERNAL_ENABLE 	  : return LGD_ERROR;		//                   
	}
...
{
	LGD_UINT16 wOperState ,wIsNullSync = 0 /*                                        */;
...
LGD_UINT8 LGD_CHANNEL_START(LGD_UINT8 ucI2CID, ST_SUBCH_INFO* pChInfo)
{
	...
	if(!wEnsemble){
		...
		if(LGD_START(ucI2CID, pChInfo, wEnsemble) != LGD_SUCCESS) return LGD_ERROR;
	}
	else{
...
LGD_DOUBLE32 LGD_GET_PREBER(LGD_UINT8 ucI2CID)
{
	...
	dPreBER = (!uiVtbData) ? 0 : (LGD_UINT16)((LGD_DOUBLE32)(uiVtbErr * 10000) / (uiVtbData * 64));	//                                 
	return dPreBER;
}
...
LGD_INT16 LGD_GET_RSSI(LGD_UINT8 ucI2CID)
{
	...
	//                                                               
	uiAdcValue  = (LGD_INT16)((1.17302*100000) * (LGD_DOUBLE32)uiAdcValue )/100000;		//                                 
