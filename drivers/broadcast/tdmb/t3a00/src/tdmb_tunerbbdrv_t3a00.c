/* drivers/broadcast/t3a00/src/tdmb_tunerbbdrv_t3a00.c
...
#include "../inc/tdmb_tunerbbdrv_t3a00def.h"
...
/*                                                           
                                                             ...
                                                             */
t3a00_service_type	serviceType;
...
void tunerbb_drv_t3a00_set_userstop(int mode)
{
	tdmb_t3a00_set_userstop( mode );
//                                         
}

int8 tunerbb_drv_t3a00_power_on(void)
{
	int rc;
	rc = tdmb_t3a00_power_on( );
	return rc;
}

int8 tunerbb_drv_t3a00_power_off(void)
{
	int rc;

	rc = tdmb_t3a00_power_off( );
	return rc;
}

int8 tunerbb_drv_t3a00_select_antenna(unsigned int sel)
{
	int rc;
	rc = tdmb_t3a00_select_antenna(sel);
	return rc;
}

int8 tunerbb_drv_t3a00_reset_ch(void)
{
...
void tunerbb_drv_t3a00_rw_test(void)
{
	unsigned short i = 0;
	...
	for(i=1;i<11;i++)
	{
		...
		if(err_cnt == 0)
		printk("t3a00 interface test ok...\n");
	}
}

int8 tunerbb_drv_t3a00_init(void)
{
	INC_UINT8 nRet;
	...
	m_ucPLL_Mode = INPUT_CLOCK_19200KHZ; //                     
	...
	if(nRet != INC_SUCCESS)
	{
		printk("[T3A00] INTERFACE_INIT Error = (%d)\n", nRet);
	}

	return nRet;
}

int8 tunerbb_drv_t3a00_stop(void)
{
	INC_UINT8 nRet;
	//                             
	nRet = INC_STOP(TDMB_RFBB_DEV_ADDR);
	if(nRet != INC_SUCCESS && (ERROR_USER_STOP==INTERFACE_ERROR_STATUS(TDMB_RFBB_DEV_ADDR)))
	{
		printk("[T3A00] INC_User STOP\n");
	}
	return nRet;
}

int8 tunerbb_drv_t3a00_get_ber(struct broadcast_tdmb_sig_info *dmb_bb_info)
{
	ST_BBPINFO* pInfo;
	int32 svc_type = 0;  /*                      */
	...
	if(T3A00_DAB == serviceType)
	{
		svc_type = 1;
	}
	else if((T3A00_DMB == serviceType) || (T3A00_VISUAL == serviceType))
	{
		svc_type = 2;
	}
	...
	if(svc_type == 2)
	{
		dmb_bb_info->va_ber = (uint32)(pInfo->ucVber);
...
int8 tunerbb_drv_t3a00_get_msc_ber(uint32 *msc_ber)
{
...
int8 tunerbb_drv_t3a00_set_channel(int32 freq_num, uint8 subch_id, uint8 op_mode)
{
	return tunerbb_drv_t3a00_multi_set_channel(freq_num, 1, &subch_id, &op_mode);
}

int8 tunerbb_drv_t3a00_re_syncdetector(uint8 op_mode)
{
	int8 ret_val = INC_ERROR;
	
	if(op_mode != T3A00_ENSQUERY)
	{		
		ret_val = INTERFACE_RE_SYNCDETECTOR(TDMB_RFBB_DEV_ADDR, &g_stSubInfo);
	}

	if(ret_val == INC_ERROR)
	{
		INC_ERROR_INFO ErrorInfo;
		
		ErrorInfo = INTERFACE_ERROR_STATUS(TDMB_RFBB_DEV_ADDR);
		printk("[INC] RESYNC INTERFACE_STATUS = (0x%04x)\n", ErrorInfo);
	}
	return ret_val;
}

int8 tunerbb_drv_t3a00_re_sync(void)
{
	return INTERFACE_RE_SYNC(TDMB_RFBB_DEV_ADDR);
}

int8 tunerbb_drv_t3a00_control_fic(uint8 enable)
{
	return INC_ERROR;
}


uint32 tunerbb_drv_t3a00_get_freq(int nFreqIndex)
{
...
int8 tunerbb_drv_t3a00_tune(int nFreqNo)
{
	uint32 ulFreq;
	INC_UINT8 nRet = INC_ERROR;

	ulFreq = tunerbb_drv_t3a00_get_freq(nFreqNo);
	//                                                                               
...
int8 tunerbb_drv_t3a00_get_fic(uint8* buffer, uint32* buffer_size/*               */)
{
		...
		else if(ret == INC_SUCCESS)
		{
			if(uiFicSize > 384)
			{
				printk("tunerbb_drv_t3a00_get_fic uiFicSize = (%d) is over 384\n", uiFicSize);
				uiFicSize = 384;
			}	
			...
			{
				printk("tunerbb_drv_t3a00_get_fic READ_BURST ERROR\n");
			}
...
int8 tunerbb_drv_t3a00_read_data(uint8* buffer, uint32* buffer_size)
{
	TDMB_BB_HEADER_TYPE dmb_header;
	uint8* r_buffer = NULL;

#if 0	
	dmb_header.data_type = (serviceType == T3A00_DAB?TDMB_BB_DATA_DAB:TDMB_BB_DATA_TS);
...
#if 0  /*                    */
	printk("tunerbb_drv_t3a00_read_data, [0x%.2X][0x%.2X][0x%.2X][0x%.2X]::[0x%.4X]::[0x%.4X]\n", 
		r_buffer[0],	r_buffer[1], 
		r_buffer[188], r_buffer[189],
		INC_CMD_READ(TDMB_RFBB_DEV_ADDR, 0x528),
		INC_CMD_READ(TDMB_RFBB_DEV_ADDR, 0x514));
#endif

	if(r_size == (uint32)INC_ERROR) {
		*buffer_size = 0;
		printk("[INC] ISR READ Size (0)!!\n");
		return INC_ERROR;
	}
...
int8 tunerbb_drv_t3a00_multi_set_channel(int32 freq_num, uint8 subch_cnt, uint8 subch_id[], uint8 op_mode[])
{
	int i;
	//                
	INC_ERROR_INFO ErrorInfo;
	...
	serviceType = (t3a00_service_type)op_mode[0];

	set_freq = tunerbb_drv_t3a00_get_freq(freq_num);

	if(op_mode[0]==T3A00_ENSQUERY)
	{
	...
	else
	{
		...
		for(i=0;i<subch_cnt;i++)
		{
			switch(op_mode[i])
			{
				...
				case T3A00_DATA:
					...
					ChInfo[i].ulDataThreshold = 288*10;//    
					break;
				case T3A00_DMB:
				case T3A00_VISUAL:
				case T3A00_BLT_TEST:
		...
		//                                  
		{
			...
			if(ErrorInfo == ERROR_SYNC_NULL || ErrorInfo == ERROR_FICDECODER || 
				ErrorInfo == ERROR_SYNC_TIMEOUT) continue; //                         
			else
			{
				return INC_ERROR;
			}
		...
		for(i=0;i<subch_cnt;i++)
		{
			...
			switch(op_mode[i])
			{
				...
				case T3A00_DATA:
					...
					_ChInfo->ulDataThreshold = 288*10;//    
					break;
				case T3A00_DMB:
				case T3A00_VISUAL:
				case T3A00_BLT_TEST:
...
int8 tunerbb_drv_t3a00_get_datatype(uint8 subchid, uint8* datatype)
{
	int i;
...
int8 tunerbb_drv_t3a00_get_datathreshold(uint8 subchid, uint32* threshold)
{
	int i;
...
int8 tunerbb_drv_t3a00_process_multi_data(uint8 subch_cnt,uint8* input_buf, uint32 input_size, uint32* read_size)
{
	ST_FIFO*	pMultiFF;
	uint32	nDataSize;
	int			i;
	...
	{
		printk("[T3A00] input_buf Error [input_buf:0x%x]",(uint32)input_buf);
		return INC_ERROR;
	}
	
	if(subch_cnt>INC_MULTI_MAX_CHANNEL) 
	{
		printk("[T3A00] subch_cnt Error [subch_cnt:%d]",subch_cnt);
		return INC_ERROR;
	}	
	...
	if(INC_MULTI_FIFO_PROCESS(input_buf, input_size))
	{
		for(i=1;i<=subch_cnt+1;i++)
		{
			...
			{
				nDataSize = INC_QFIFO_GET_SIZE(pMultiFF);
				if(tunerbb_drv_t3a00_get_datathreshold(pMultiFF->unSubChID, &threshold)==INC_ERROR)
				{
					return INC_ERROR;
				}
			}
	...
	{
		printk("[T3A00] INC_MULTI_FIFO_PROCESS() Error [input_buf:0x%x, input_size:%d]",(uint32)input_buf,input_size);
		return INC_ERROR;
	}
...
int8 tunerbb_drv_t3a00_get_multi_data(uint8 subch_cnt, uint8* buf_ptr, uint32 buf_size)
{
	ST_FIFO*	pMultiFF;
	uint32	nDataSize;
	int			i;
	...
	for(i=1;i<=subch_cnt+1;i++)
	{
		...
		{
			nDataSize = INC_QFIFO_GET_SIZE(pMultiFF);
			if(tunerbb_drv_t3a00_get_datatype(pMultiFF->unSubChID, &datatype)==INC_ERROR)
			{
				return INC_ERROR;
			}
			if(tunerbb_drv_t3a00_get_datathreshold(pMultiFF->unSubChID, &threshold)==INC_ERROR)
		...
		{
			printk("[T3A00] skip INC_QFIFO_BRING()  [i:%d, nDataSize:%d, threshold:%d]",read_size,nDataSize,threshold);
		}
	}

	if(read_size != buf_size)
	{
		printk("[T3A00] tunerbb_drv_t3a00_get_multi_data() Error [read_size:%d, buf_size:%d]",read_size,buf_size);
		return INC_ERROR;		
	}
	else
	{
...
void tunerbb_drv_t3a00_start_tii(void)
{
	INC_TII_START(TDMB_RFBB_DEV_ADDR);
	//                                            
}

void tunerbb_drv_t3a00_stop_tii(void)
{
	INC_TII_STOP(TDMB_RFBB_DEV_ADDR);
	//                                         
}

bool tunerbb_drv_t3a00_check_tii(uint8* pmain_tii, uint8* psub_tii)
{
...
int8 tunerbb_drv_t3a00_get_fic(uint8* buffer, uint32* buffer_size /*                   */)
{
	...
	{
		*buffer_size = uiFicSize;
		printk("tunerbb_drv_t3a00_get_fic = %x %x %x %x %x\n", *buffer, *(buffer+1), *(buffer+2), *(buffer+3), *(buffer+4));
		return INC_SUCCESS;
	}
