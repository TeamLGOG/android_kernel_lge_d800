...
void INC_UPLOAD_MODE(INC_UINT8 ucI2CID)
{
	...
	if(m_ucCommandMode != INC_EBI_CTRL){
		...
		//                                              
		if(m_ucCommandMode == INC_I2C_CTRL) 				 INC_CMD_WRITE(ucI2CID, APB_SPI_BASE+ 0x00, 0x0011);
...
INC_UINT8 INC_PLL_SET(INC_UINT8 ucI2CID) 
{
	...
	case INPUT_CLOCK_24576KHZ:
		...
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x02, 0x000F);	//                                    
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x03, 0x0000);	
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x04, 0x0000);	
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x10, 0x0162);   //             
	break;
	case INPUT_CLOCK_19200KHZ:
		//                                                                       
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x00, 0x0000);	//                    
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x01, 0x0380);	
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x02, 0x03D3);	
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x03, 0x70A3);	
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x04, 0xD740);	

	break;
	default:
		printk("[INC]Not Support ucPLL clock=(%d)\n", m_ucPLL_Mode);
		return INC_ERROR;	
	}
	...
	/*                  */
	INC_MUST_DELAY(10);
	uiDpll = INC_CMD_READ(ucI2CID, APB_GEN_BASE+ 0x05);
	if(uiDpll != 0x0001){
	...
	INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x10, 0x0422);	 //             

	uiDpll = INC_CMD_READ(ucI2CID, APB_GEN_BASE+0x10);
	if(uiDpll != 0x0422)
	{
		printk("[INC] DPLL Set Error uiDpll =(0x%04x)\n", uiDpll);
		pInfo->nBbpStatus = ERROR_PLL;
		return INC_ERROR;
	}
...
INC_UINT8 INC_INIT(INC_UINT8 ucI2CID)
{
	...
	if(m_ucCommandMode == INC_SPI_CTRL)	
		INC_CMD_WRITE(ucI2CID, APB_MPI_BASE+ 0x00, 0x0005);


	if(INC_CHIP_STATUS(ucI2CID) != INC_SUCCESS)
	{
		printk("[INC] T3A00 Chip ID Read Fail.\n");
		return INC_ERROR;
	}
	if(INC_PLL_SET(ucI2CID) != INC_SUCCESS) 		
	{
		printk("[INC] T3A00 PLL INIT Fail.\n");
		return INC_ERROR;
	}
	...
	INC_CMD_WRITE(ucI2CID, APB_PHY_BASE+ 0xF4, 0xC31F);
	//                                                   
...
{
	ST_BBPINFO* pInfo;
	INC_UINT8 byIsChange = 0;
	pInfo = INC_GET_STRINFO(ucI2CID);

	if( ulFreq == 196736 ) {
		if( m_ucPLL_Mode == INPUT_CLOCK_24576KHZ) {
			if(INC_CMD_READ(ucI2CID, APB_GEN_BASE+ 0x01) != 0x880)	{
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x00, 0x0000);
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x01, 0x0880);  
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x02, 0x0A23);  
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x03, 0xC2AA);  
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x04, 0xAA50); 
				byIsChange = 1;
			}
		}
		else if(m_ucPLL_Mode == INPUT_CLOCK_19200KHZ) {
			if(INC_CMD_READ(ucI2CID, APB_GEN_BASE+ 0x01) != 0x880)	{
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x00, 0x0000);
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x01, 0x0880);
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x02, 0x0D03);  
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x03, 0x5556);  
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x04, 0x3850);  
				byIsChange = 1;
			}
		}
		...
		if(!(INC_CMD_READ(ucI2CID, APB_GEN_BASE+ 0x05) & 0x0001))
		{
			printk("[INC]INC_READY ERROR\n");
			return INC_ERROR;
		}

		INC_CMD_WRITE(ucI2CID, APB_PHY_BASE+ 0xF9, 0x046A);
		}
	}
	else {
		if( m_ucPLL_Mode == INPUT_CLOCK_24576KHZ) {
			if(INC_CMD_READ(ucI2CID, APB_GEN_BASE+ 0x01) != 0x0001)	{
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x00, 0x0000);
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x01, 0x0001);   //              
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x02, 0x000F);   //                                    
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x03, 0x0000);   
			...
			if(INC_CMD_READ(ucI2CID, APB_GEN_BASE+ 0x01) != 0x0380)	{
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x00, 0x0000);
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x01, 0x0380);   
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x02, 0x03D3);   
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x03, 0x70A3);   
...
INC_UINT8 INC_SYNCDETECTOR(INC_UINT8 ucI2CID, INC_UINT32 ulFreq, INC_UINT8 ucScanMode)
{
	...
	while(1) {
		...
		if (!wIsNullSync) {
			if((awData[2] < 0x1 || awData[2] > 0x6) && awData[0] != 0x400) {
		...
		if ((wIsNullSync == 1) && uiTimeOut >= (600 / uiRefSleep)){
			if (awData[0] == 0) {
		...
		if ((wIsNullSync == 1) && uiTimeOut < (1500 / uiRefSleep)){
			if(awData[1] <= 0x6000 && awData[1] && awData[2] >= 7){
...
INC_UINT8 INC_FICDECODER(INC_UINT8 ucI2CID, ST_SIMPLE_FIC bSimpleFIC)
{
	...
	for(nLoop = 0; nLoop < nFicCnt; nLoop++)
	{
		...
		if(!(INC_CMD_READ(ucI2CID, APB_VTB_BASE+ 0x0F) & 0x0002))
			continue;
...
{
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0x1f, 0x0D);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0x28, 0x05);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0x2F, 0x3F);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0x8E, 0x20);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xA1, 0x80);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xA2, 0x00);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xAF, 0x46);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xB0, 0x46);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xB1, 0x46);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xB2, 0xC4);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xB3, 0x47);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xC9, 0x01);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xCD, 0x70);
	...
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xA2, 0xA0);
#endif		
}
...
INC_UINT8 INC_CHANNEL_START(INC_UINT8 ucI2CID, ST_SUBCH_INFO* pChInfo)
{
	...
	if(!wEnsemble){
		...
		if(INC_SYNCDETECTOR(ucI2CID, ulRFFreq, 0) != INC_SUCCESS){
			pInfo->ulFreq = 0;
			printk("[INC] ENS INC_SYNCDETECTOR Return \n");
			return INC_ERROR;
		}
		if(INC_FIC_UPDATE(ucI2CID, pChInfo, SIMPLE_FIC_ENABLE) != INC_SUCCESS)
		{
		...
		if(INC_START(ucI2CID, pChInfo, wEnsemble) != INC_SUCCESS)
		{
			printk("[INC] ENS INC_START Return\n");
			return INC_ERROR;
		}
	}
...
INC_UINT8 INC_CHANNEL_START(INC_UINT8 ucI2CID, ST_SUBCH_INFO* pChInfo)
{
	...
	wEnsemble = pInfo->ulFreq == ulRFFreq;
#if 0
	INC_MSG_PRINTF(1, "================= T3A00 INC_CHANNEL_START =================\r\n");
...
*/
	...
	/*                              */
	/*                  */
	if(/*                            */(unRefAntLevel == 0) && (pInfo->uiCER < 1300) && (pInfo->ucVber >= 50))
	 unRefAntLevel+=1;

	if(/*                            */(unRefAntLevel == 1) && (pInfo->ucVber < 50))
		unRefAntLevel-=1;

	if(/*                            */(unRefAntLevel == 2) && (pInfo->ucVber <= 50))
	  unRefAntLevel -=1;
	/*     */
