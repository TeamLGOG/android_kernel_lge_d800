...
#define T3A00_USES_STATIC_BUFFER

//                                                                                                
#define TDMB_MPI_BUF_SIZE 			(8120)   /*                                              */
...
static unsigned int 	s_opmode = T3A00_SERVICE_MAX;
static uint32           g_set_channel_called = 0;  /*                                                */

#ifdef T3A00_USES_STATIC_BUFFER
...
boolean broadcast_drv_if_read_data(void)
{
	...
	if(g_set_channel_called == 0)
	{
		printk("[INC]Ch Setting is not called. MSC Read Skip\n");
		return FALSE;
	}

	read_buffer_ptr = gpMPI_Buffer + gBBBuffer_widx*TDMB_MPI_BUF_SIZE;
	tunerbb_drv_t3a00_read_data(read_buffer_ptr, &read_buffer_size);

	if(gBBBuffer_ridx == ((gBBBuffer_widx + 1)%TDMB_MPI_BUF_CHUNK_NUM))
	{	
		//                                                   
		if(s_opmode!=T3A00_BLT_TEST)
		{
...
int broadcast_drv_if_power_on(void)
{
	...
	g_set_channel_called = 0;
	if(gpMPI_Buffer == NULL)
	{
#ifndef T3A00_USES_STATIC_BUFFER
	...
	retval = tunerbb_drv_t3a00_power_on( );
...
int broadcast_drv_if_power_off(void)
{
	...
	retval = tunerbb_drv_t3a00_power_off( );
	g_set_channel_called = 0;
...
int broadcast_drv_if_open(void) 
{
	...
	g_set_channel_called = 0;
	retval = tunerbb_drv_t3a00_init( );

	if(retval == FALSE)
	{
		/*                    */
		printk("[T3A00]broadcast_drv_if_open failed retry\n");
...
int broadcast_drv_if_set_channel(unsigned int freq_num, unsigned int subch_id, unsigned int op_mode)
{
	...
	g_set_channel_called = 1;
	retval = tunerbb_drv_t3a00_set_channel(freq_num, subch_id, op_mode);
...
int broadcast_drv_if_detect_sync(int op_mode)
{
	...
	retval = tunerbb_drv_t3a00_re_syncdetector(op_mode);
...
int broadcast_drv_if_get_sig_info(struct broadcast_tdmb_sig_info *dmb_bb_info)
{
	...
	retval = tunerbb_drv_t3a00_get_ber(dmb_bb_info);
...
int broadcast_drv_if_get_ch_info(char* buffer, unsigned int* buffer_size)
{
	...
	retval = tunerbb_drv_t3a00_get_fic(buffer, buffer_size);
...
int broadcast_drv_if_reset_ch(void)
{
	...
	retval = tunerbb_drv_t3a00_reset_ch( );
...
{
	tunerbb_drv_t3a00_set_userstop( mode );
	return OK ;
}

int broadcast_drv_if_select_antenna(unsigned int sel)
{
	tunerbb_drv_t3a00_select_antenna(sel);
	return OK;
}
