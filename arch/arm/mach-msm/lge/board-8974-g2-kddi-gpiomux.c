...
if (socinfo_get_version() >= 0x20000) {
 ...
 msm_tlmm_misc_reg_write(TLMM_SPARE_REG, 0x7);
	...
	if (lge_get_board_revno() >= HW_REV_C)
		msm_gpiomux_install(msm8974_nfc_configs_revc, ARRAY_SIZE(msm8974_nfc_configs_revc));
	else
		msm_gpiomux_install(msm8974_nfc_configs_revb, ARRAY_SIZE(msm8974_nfc_configs_revb));
